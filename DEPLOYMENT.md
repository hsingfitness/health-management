...
| Stripe (combined checkout)      | ⬜ No Stripe keys set. (Note: the site's cart *already* works today via a separate, simpler method — backend-created Stripe Checkout Sessions. This backend's combined-cart checkout is an upgrade path, not a blocker.) |
